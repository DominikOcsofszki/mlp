...
def show_scatter_binary_dataset(model: nn.Module, mydataset_subset: MyDataSets_Subset,
                                current_epoch='epoch_information'):
    model.eval()
...
def show_scatter_with_batch_from_iter(model: nn.Module, batch_from_dataloader_iter, current_epoch='epoch_information'):
    # model.eval()

    # testing_images, labels = mydataset_subset.dataloader_test_subset_one_batch()
    testing_images, labels = batch_from_dataloader_iter

    mu, sigma = model.encode(testing_images)
    # TODO only used mus, but with sigma it
    # z = mu + sigma
    z = mu
    print(f'{z.mean() = }')
    z_det = z.detach().numpy()
    ...
    # plt.scatter(z_det[:, 0], z_det[:, 1], c=labels, cmap='Dark2_r', alpha=0.9)
    # plt.scatter(z_det[:, 0], z_det[:, 1], c=labels, cmap='tab10', alpha=0.9)
    plt.scatter(z_det[:, 0], z_det[:, 1], c=labels, cmap='coolwarm', alpha=0.9)
...
def latent_rand_to_img_ret_z(modelpick, tensor_rand):
    rand_lat = tensor_rand
    z = modelpick.decode(rand_lat)
    z_reshaped = z.view(28, 28)
    znew = z_reshaped.detach()
...
def print_range_of_lattent_to_img(modelpick, min=-4., max=4, plot_elements=10):
    return ''
